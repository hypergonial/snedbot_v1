            ...
            if not retry_after and len(message.content) < 1500:  # If not ratelimited
                ...
                if message.content in mentions:
                    ...
                    embed = discord.Embed(
                        title=_("Beep Boop!"),
                        description=_(
                            "My prefixes on this server are the following: `{prefix}` \nUse the command `{prefix_0}help` to see what I can do!"
                        ).format(prefix="`, `".join(prefix), prefix_0=prefix[0]),
                        color=0xFEC01D,
                    )
        ...
        if guild.system_channel is not None:
            try:
                embed = discord.Embed(
                    title=_("Beep Boop!"),
                    description=_(
                        "I have been summoned to this server. Use `{prefix}help` to see what I can do!"
                    ).format(prefix=bot.DEFAULT_PREFIX),
                    color=0xFEC01D,
                )
                embed.set_thumbnail(url=self.user.avatar.url)
        ...
        if isinstance(error, commands.CheckFailure):
            ...
            if isinstance(error, commands.BotMissingPermissions):
                embed = discord.Embed(
                    ...
                    description=_(
                        "The bot requires additional permissions to execute this command.\n**Error:**```{error}```"
                    ).format(error=error),
            ...
            embed = discord.Embed(
                title=self.errorCooldownTitle,
                description=_("Please retry in: `{cooldown}`").format(
                    cooldown=datetime.timedelta(seconds=round(error.retry_after))
                ),
                color=self.errorColor,
            )
            embed.set_footer(
            ...
            embed = discord.Embed(
                ...
                description=_(
                    "One or more arguments are missing. \n__Hint:__ You can use `{prefix}help {command_name}` to view command usage."
                ).format(prefix=ctx.prefix, command_name=ctx.command.qualified_name),
                color=self.errorColor,
            )
            embed = self.add_embed_footer(ctx, embed)
            logging.info(
            ...
            embed = discord.Embed(
                title=self.errorMaxConcurrencyReachedTitle,
                description=self.errorMaxConcurrencyReachedDesc,
                color=self.errorColor,
            )
            embed = self.add_embed_footer(ctx, embed)
            ...
            embed = discord.Embed(
                title="❌ " + _("Cannot find user by that name"),
                description=_(
                    "Please check if you typed everything correctly, then try again.\n**Error:**```{error}```"
            ...
            embed = discord.Embed(
                ...
                description=_(
                    "Invalid data entered! Check `{prefix}help {command_name}` for more information.\n**Error:**```{error}```"
                ).format(
                    prefix=ctx.prefix,
            ...
            embed = discord.Embed(
                ...
                description=_(
                    "You have provided more arguments than what `{prefix}{command_name}` can take. Check `{prefix}help {command_name}` for more information."
                ).format(prefix=ctx.prefix, command_name=ctx.command.qualified_name),
                color=self.errorColor,
            )
            embed = self.add_embed_footer(ctx, embed)
            ...
            embed = discord.Embed(
                ...
                description=_(
                    "An error happened that should not have happened. Please [contact us](https://discord.gg/KNKr8FPmJa) with a screenshot of this message!\n**Error:** ```{error}```"
                ).format(error=error),
                color=self.errorColor,
            )
...
bot.errorCheckFailDesc = _(
    "You did not meet the checks to execute this command. This could also be caused by incorrect configuration. \nType `{prefix}help` for a list of available commands."
)
