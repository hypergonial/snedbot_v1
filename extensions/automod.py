...
class AutoMod(commands.Cog, name="Auto-Moderation"):
    ...
    async def automod_punish(
        self,
        message,
        offender: discord.Member,
        offense: str,
        reason: str,
        original_offense: str = None,
    ):
        """
        ...
        if policy_state == "warn":
            await self.mod_cog.warn(
                ctx,
                offender,
                ctx.guild.me,
                reason=f"Warned by auto-moderator for {reason}.",
            )
            ...
            if bucket.update_rate_limit():
                ...
                await self.automod_punish(
                    ...
                    offender,
                    offense="escalate",
                    reason=f"previous offenses ({offense})",
                    original_offense=offense,
                )
                ...
                await self.mod_cog.warn(
                    ...
                    offender,
                    ctx.guild.me,
                    reason=f"Warned by auto-moderator for previous offenses. ({offense})",
                )
            else:
            ...
            await self.mod_cog.timeout(
                ctx,
                offender,
                ctx.guild.me,
                duration=f"{temp_dur} minutes",
            ...
            await self.mod_cog.kick(
                ctx,
                offender,
                ctx.guild.me,
                reason=f"Kicked by auto-moderator for {reason}.",
            )

        elif policy_state == "softban":
            await self.mod_cog.ban(
                ctx,
                offender,
                ctx.guild.me,
                soft=True,
                reason=f"Soft-banned by auto-moderator for {reason}.",
            )
            ...
            await self.mod_cog.ban(
                ...
                offender,
                ctx.guild.me,
                reason=f"Permanently banned by auto-moderator for {reason}.",
            )
                ...
                for bad_word in policies["bad_words"][
                    ...
                    if " " in bad_word and bad_word in message.content:
                        return await self.automod_punish(
                            ...
                            offender=message.author,
                            offense="bad_words",
                            reason=f"usage of bad words (expression)",
                        )
                    else:
                        for word in policies["bad_words"]["words_list_wildcard"]:
                            if word in message.content:
                                return await self.automod_punish(
                                    ...
                                    offender=message.author,
                                    offense="bad_words",
                                    reason=f"usage of bad words (wildcard)",
                                )
            ...
            if invite_matches:
                ...
                if bucket.update_rate_limit():
                    await self.automod_punish(
                        ...
                        offender=message.author,
                        offense="attach_spam",
                        reason="posting images/attachments too quickly",
                    )
            else:  # Check zalgo
                count = 0
