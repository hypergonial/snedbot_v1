...
class Listings:
    ...
    async def retrieve(self, id):
        ...
        if len(result) != 0 and result[0]:
            listing = Listing(
                id=result[0].get("id"),
                ubiname=result[0].get("ubiname"),
                host_id=result[0].get("host_id"),
                gamemode=result[0].get("gamemode"),
                playercount=result[0].get("playercount"),
                DLC=result[0].get("DLC"),
                mods=result[0].get("mods"),
                timezone=result[0].get("timezone"),
                additional_info=result[0].get("additional_info"),
...
class Matchmaking(commands.Cog):
    ...
    async def matchmaking_conf(self, ctx):
        ...
        try:
            ...
            ON CONFLICT (guild_id) DO
            UPDATE SET init_channel_id = $2, announce_channel_id = $3, lfg_role_id = $4""",
                ctx.guild.id,
                cmdchannel_id,
                announcechannel.id,
                lfg_role_id,
    ...
    @commands.command(
        help="Start setting up a new multiplayer listing.",
        description="Start matchmaking! After command execution, you will receive a direct message to help you set up a multiplayer listing! Takes no arguments.",
        aliases=["multiplayer", "init", "match", "multi", "mp"],
        usage=f"matchmaking",
    )
    @commands.guild_only()
    ...
    async def matchmaking(self, ctx):
        ...
        embed = discord.Embed(
            title=self._("**Hello!**"),
            description=self._(
                "I will help you set up a new multiplayer listing!  Follow the steps below! \n__Note:__ You can edit your submission at the end in case you made any errors!"
            ),
            color=mpEmbedColor,
        )
        embed.set_thumbnail(
            url="https://cdn.discordapp.com/avatars/203158031511453696/446da0b60a670b6866cd463fb5e87195.png?size=1024"
        )
        ...
        async def ask(qType, isModifying):
            ...
            if qType == "UbiName":
                embed = discord.Embed(
                    title=self._("Ubisoft Connect username"),
                    description=self._("Please type in your Ubisoft Connect username!"),
                    color=mpEmbedColor,
                )
                ...
                embed = discord.Embed(
                    title=self._("Choose your preferred style of play!"),
                    description="⚔️ - PvP (Player versus Player) \n🛡️ - PvE (Players versus Environment)\n⛏️ - Co-Op (Cooperative)",
                    color=mpEmbedColor,
                )
                ...
                embed = discord.Embed(
                    title=self._(
                        "Now react with the symbol of **all** the DLCs you want to use! Click the green checkmark ({checkmark}) once done!"
                    ).format(checkmark="✅"),
                    description=self._(
                ...
                DLCemojies = ["🔥", "🤿", "🌹", "❄️", "🏛️", "🚜", "🦁", "⚓", "🏖️", "✅"]
                allDLCs = [
                    self._("The Anarchist"),
                    self._("Sunken Treasures"),
                    self._("Botanica"),
                    self._("The Passage"),
                    self._("Seat of Power"),
                    self._("Bright Harvest"),
                    self._("Land of Lions"),
                    self._("Docklands"),
                    self._("Tourist Season"),
                ]
                ...
                # Add msg
                embed = discord.Embed(
                    title=self._("Are you going to use mods in this match?"),
                    description=self._("React below with your response!"),
                    color=mpEmbedColor,
                )
                embed.set_footer(
                    text=self._(
                        "Note: Mods are not officially supported. All participants must share the same mods to play together. Please share the mods you want to use at the end of the form."
                    )
                )
                msg = await ctx.author.send(embed=embed)
                # Add emoji
                ...
                embed.set_footer(
                    text=self._(
                        "If you are unsure what timezone you are in, you can check here: https://www.timeanddate.com/time/map"
                    )
                )
                msg = await ctx.author.send(embed=embed)
                ...
                try:
                    ...
                    if len(payload.content) > 256:
                        ...
                        if payload.content.lower() == "skip":
                            ...
                            await msg.delete()
                            embed = discord.Embed(
                                title="✅ " + self._("Additional info skipped."),
                                description=self._("You skipped this step."),
                                color=mpEmbedColor,
                            )
                            await ctx.author.send(embed=embed)
                            return 0
                ...
                embed = discord.Embed(
                    title=self._("**__Looking for Players: Anno 1800__**"),
                    description=self._(
                        "**Ubisoft Connect Username: ** {name} \n **Gamemode: ** {gamemode} \n **Players: ** {playercount} \n **DLC: ** {DLC} \n **Mods:** {mods} \n **Timezone:** {timezone} \n **Additional info:** {additional_info} \n \n Contact {author} in DMs if you are interested, or subscribe by reacting with {arrow}! This will notify the host when {subcap} players have subscribed! (including the host)"
                    ).format(
                        name=mpsessiondata[0],
                        gamemode=mpsessiondata[1],
                        playercount=mpsessiondata[2],
                        DLC=mpsessiondata[3],
                        mods=mpsessiondata[4],
                        timezone=mpsessiondata[5],
                        additional_info=mpsessiondata[6],
                        author=ctx.author.mention,
                        arrow="⏫",
                        subcap=mpsessiondata[2],
                    ),
                    color=mpEmbedColor,
                )
                embed.set_thumbnail(
                    url="https://cdn.discordapp.com/attachments/310078048525615106/830900971680038962/union_crop.jpg"
                )
                embed.set_footer(
                    text=self._(
                        "Note: This listing is valid for 7 days, after that, no more subscriptions can be submitted."
                    )
                )
                await ctx.author.send(embed=embed)
                embed = discord.Embed(
                    title=self._("Please review your listing!"),
                    description=self._(
                        "If everything looks good, hit {check} to submit! If you want to edit any information, hit {pen}. If you want to cancel your submission, hit {cross}."
                    ).format(check="✅", pen="🖊️", cross="❌"),
                    color=mpEmbedColor,
                )
                msg = await ctx.author.send(embed=embed)
                await msg.add_reaction("✅")
                ...
                async def createposting(mpsessiondata):
                    try:
                        ...
                        if lfgrole_id == None:
                            ...
                            embed.set_footer(
                                text="Note: This listing is valid for 7 days, after that, no more subscriptions can be submitted.\nID: {listing_id}".format(
                        ...
                        logger.error(
                            f"Could not create listing for {ctx.author} due to unhandled exception. Did you set up matchmaking?"
                        )
                        embed = discord.Embed(
                            title="❌ " + self._("Error: Exception encountered."),
                            description=self._(
                ...
                async def modifylisting():
                    embed = discord.Embed(
                        title=self._("What do you want to change in your listing?"),
                        description=self._(
                            "{em} - Ubisoft Username \n{em0} - Gamemode \n{em1} - Player count \n{em2} - DLC \n{em3} - Mods\n{em4} - Timezone \n{em5} - Additional details"
                        ).format(
                            em="👤",
                            em0="🕹️",
                            em1="🧮",
                ...
                try:
                    ...
                    if str(payload.emoji) == "✅":
                        if await createposting(mpsessiondata) == -1:
                            ...
                            embed = discord.Embed(
                                title="✅ " + self._("Listing submitted!"),
                                description=self._(
                        ...
                        if await modifylisting() == 0:
                            ...
                            embed = discord.Embed(
                                title="❌ " + self._("Modification failed."),
                                description=self._(
                        ...
                        embed = discord.Embed(
                            title="❌ " + self._("Submission cancelled."),
                            description=self._(
                                "If you have found a bug or want to give feedback, please contact `Hyper#0001`!"
                            ),
                            color=self.bot.error_color,
        ...
        while errcode != 1:
            ...
            # If editing, we will add one to the edits variable.
                ...
                if edits == 6:
                    embed = discord.Embed(
                        title="❌ " + self._("Exceeded edit limit."),
                        description=self._(
            ...
            if await self.config.load("announce_channel_id", payload.guild_id) == payload.channel_id:
                ...
                if str(payload.emoji) == "⏫" and payload.user_id != self.bot.user.id:
                    ...
                    if len(interestedPlayers) >= playerCount:
                        ...
                        await host.send(
                            embed=embed,
                            content=f"Players that want to play with you: {interestedMentions}",
                        )
