...
class Permissions(commands.Cog):
    ...
    @commands.group(
        help="View server permissions. See subcommands for modifying them.",
        description="Provides a detailed overview of all server permissions. You can edit them with the subcommands below.",
    ...
    async def permissions(self, ctx):
        embed = discord.Embed(
            title="🛠️ Permissions Configuration",
            description=f"""These are the current permission nodes and the roles assigned to them.
        ...
        if records:
            ...
            for record in records:
                ...
                if role_ids and len(role_ids) > 0:
                    ...
                    embed.add_field(
                        name=self.VALID_TYPES[record["ptype"]],
                        value=str(self.DEFAULT_PERMS[record["ptype"]]),
                    )
    ...
    @permissions.command(
        name="info",
        help="Provides information about a specified permission node.",
        description="Provides detailed information about a specified permission node.",
        usage="permission info <node>",
    )
    @commands.guild_only()
    @commands.check(has_admin_perms)
    ...
    @permissions.command(
        name="add",
        aliases=["new"],
        help="Adds a role to a permission-node.",
        description="Adds the specified role to the permission-node.",
        usage="permission add <node> <role>",
    )
    @commands.guild_only()
    @commands.check(has_admin_perms)
    ...
    @permissions.command(
        name="delete",
        aliases=["del", "remove"],
        help="Removes a role from a permission-node.",
        description="Removes specified role from the permission-node.",
        usage="permission delete <node> <role>",
    )
    @commands.guild_only()
    @commands.check(has_admin_perms)
