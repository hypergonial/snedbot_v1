...
class RoleButtons(commands.Cog, name="Role-Buttons"):
    ...
    @commands.group(aliases=["rr", "rb", "reactionrole"], help="Manages role-buttons.", description="Lists all button roles set for this guild, if any. Subcommands allow you to remove or set additional ones.", usage="buttonrole", invoke_without_command=True, case_insensitive=True)
    @commands.guild_only()
    ...
    async def rb_setup(self, ctx):
        ...
        if has_msg == False:
            ...
            await view.wait()
            if view.value:
                reactchannel = ctx.guild.get_channel(int(view.value["values"][0]))
            else:
        ...
        await view.wait()
        if view.value:
            reactionrole = ctx.guild.get_role(int(view.value["values"][0]))
        else:
            raise asyncio.exceptions.TimeoutError
