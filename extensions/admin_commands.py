...
class AdminCommands(commands.Cog, name="Admin Commands"):
    ...
    @commands.command(
        help="Resets all settings for this guild.",
        aliases=["reset"],
        description="Resets all settings for this guild. Requires priviliged access and administrator permissions. Will also erase all tags, reminders, reaction roles and pending moderation actions. Irreversible.",
        usage="resetsettings",
    )
    ...
    @commands.command(
        help="Sets the bot's nickname.",
        description="Sets the bot's nickname for this server. Provide `Null` or `None` to reset nickname.",
    ...
    @commands.group(
        aliases=["prefixes"],
        help="Check the bot's prefixes. Subcommands of this command allow you to customize your prefix.",
        description="Check the bot's prefixes. You can also use `add/del` to add or remove a prefix. By adding a prefix you override the default one. The bot can have up to **5** custom prefixes per server. If you forget your prefix, mention the bot!",
        usage="prefix",
    ...
    @prefix.command(
        name="add",
        aliases=["new"],
        help="Adds a new prefix.",
        description="Adds a prefix to the list of valid prefixes.",
    ...
    async def add_prefix(self, ctx, *, prefix: str):
        ...
        if (
            ...
            or (prefix not in records[0]["prefix"] and len(records[0]["prefix"]) <= 5)
        ):
    ...
    @prefix.command(
        ...
        help="Removes a prefix.",
        description="Removes a prefix from the list of valid prefixes.",
        usage="prefix del <prefix>",
    )
    @commands.guild_only()
    ...
    @commands.command(
        help="Run a command while bypassing checks and cooldowns.",
        description="Run a specified command while bypassing any checks and cooldowns. Requires server administator permissions for the user to run this command alongside priviliged access.",
    ...
    async def sudo(self, ctx, *, command):
        ...
        disabled_list = [
            ...
            "leave",
            "automoderation",
        ]  # Stuff that literally does not work
        disabled_cogs = [
            "Events",
            "Permissions",
            "Annoverse",
            ...
            "Keep On Top",
            "Setup",
        ]  # Entire cogs can be disabled too
    ...
    @commands.command(
        ...
        description="Shuts the bot down properly and closes all pending connections.",
        usage="shutdown",
    )
    @commands.is_owner()
    ...
    @commands.command(
        help="Forces the bot to leave this server.",
        description="Forces the bot to leave this server. Takes no arguments.",
        usage="leave",
    )
    @commands.is_owner()
    @commands.guild_only()
    ...
    @commands.group(
        help="Blacklists a member from using the bot.",
        description="Blacklists a member from using any of the bot's commands. To query a user's blacklisted status, use the `whois` command.",
    ...
    @blacklist.command(
        ...
        help="Removes a member from the blacklist.",
        usage="blacklist del <user>",
    )
    @commands.is_owner()
    ...
    @commands.command(
        help="Query guild data",
        description="Query guild data for any guild the bot is in.",
        usage="guildinfo <ID>",
    )
    @commands.is_owner()
    async def guildinfo(self, ctx, guild_id: int):
        ...
        if guild:
            embed = discord.Embed(
                title="ℹ️ " + self._("Server information"),
                description=self._(
                    """**Name:** `{guild_name}`
            **ID:** `{guild_id}`
            **Owner:** `{owner}`
            ...
            **Nitro Boost level:** `{premium_tier}`"""
            ...
            ):  # If the server has a discovery splash/invite background, we put it as an embed image for extra fancyTM
