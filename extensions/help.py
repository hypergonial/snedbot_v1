...
class SnedHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        *Do not include the brackets in your commands!*        
        ...
        ''')
        help_home_embed.set_footer(text=ctx.bot.requestFooter.format(user_name=ctx.author.name, discrim=ctx.author.discriminator), icon_url=ctx.author.avatar_url)
    ...
    async def send_command_help(self, command):
        ...
        self._ = ctx.bot.get_localization('help', ctx.bot.lang)
        if command.parents:
            detail_embed=discord.Embed(title="⚙️ " + self._("Command: {prefix}{parent} {command}").format(prefix=self.clean_prefix, parent=command.full_parent_name, command=command.name), color=ctx.bot.embedBlue)
        else:
            detail_embed=discord.Embed(title="⚙️ " + self._("Command: {prefix}{command}").format(prefix=self.clean_prefix, command=command.name), color=ctx.bot.embedBlue)
        ...
        if command.usage:
            detail_embed.add_field(name=self._("Usage:"), value=f"**`{self.clean_prefix}{command.usage}`**", inline=False) #Getting command usage & formatting it
        aliases = []
        ...
        detail_embed.set_footer(text=ctx.bot.requestFooter.format(user_name=ctx.author.name, discrim=ctx.author.discriminator), icon_url=ctx.author.avatar_url)
    ...
    async def send_cog_help(self, cog):
        ...
        embed.set_footer(text=ctx.bot.requestFooter.format(user_name=ctx.author.name, discrim=ctx.author.discriminator), icon_url=ctx.author.avatar_url)
    ...
    async def send_group_help(self, group):
        ...
        if group.usage:
            group_embed.add_field(name=self._("Usage:"), value=f"**`{self.clean_prefix}{group.usage}`**", inline=False) #Getting command usage & formatting it
        aliases = []
