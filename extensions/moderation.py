...
class Moderation(commands.Cog):
    ...
    async def on_timeout_extend_timer_complete(self, timer):
        ...
        try:
            ...
            if soft:
                ...
                try:
                    ...
                    embed = discord.Embed(
                        title="❌ " + self._("Tempbanning failed."),
                        description=self._(
    ...
    @commands.group(
        name="journal",
        aliases=["note", "notes"],
        help="Manage the moderation journal of a user.",
        description="Manage the moderation journal of a user. Useful for logging behaviour related to a user.",
        usage="journal <user>",
    ...
    @notes_cmd.command(
        name="add",
        help="Add a new journal entry for the user.",
        description="Adds a new manual journal entry for the specified user.",
        usage="journal add <user> <note>",
    )
    @commands.check(has_mod_perms)
    @commands.guild_only()
    ...
    @commands.group(
        name="warn",
        help="Warns a user. Subcommands allow you to clear warnings.",
        aliases=["bonk"],
        description="Warns the user and logs it.",
    ...
    @warn_cmd.command(
        name="clear",
        help="Clears all warnings from the specified user.",
        aliases=["clr"],
    )
    @commands.check(has_mod_perms)
    @commands.guild_only()
    ...
    async def warn_clr(self, ctx, offender: discord.Member, *, reason: str = None):
        ...
        await self.add_note(
            offender.id,
            ctx.guild.id,
            f"⚠️ **Warnings cleared by {ctx.author}:** {reason}",
        )
        try:
            await self.bot.get_cog("Logging").log("warn", warnembed, ctx.guild.id)
            await ctx.send(embed=embed)
    ...
    @commands.group(
        ...
        description='Times out a user for the specified duration. Logs the event if logging is set up. Please note that if you wish to separate the duration by spaces, you must wrap it in quotation marks.\n**Example:** `"10 days"` or `1week`',
        ...
        if not member.timed_out:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description="**{offender}** has been timed out until {duration}.\n**Reason:** ```{reason}```".format(
                        offender=member,
    ...
    @timeout_cmd.command(
        ...
        description="Removes timeout from a user. Logs the event if logging is set up.",
    ...
    @commands.command(
        name="ban",
        help="Bans a user.",
        description="Bans a user with an optional reason. Deletes the last 7 days worth of messages from the user.",
    ...
    @commands.command(
        name="unban",
        help="Unbans a user.",
        description="Unbans a user with an optional reason. Deletes the last 7 days worth of messages from the user.",
    ...
    async def unban_cmd(self, ctx, offender: discord.User, *, reason: str = None):
        ...
        try:
            ...
            await self.add_note(
                offender.id,
                ctx.guild.id,
                f"🔨 **Unbanned by {ctx.author}:** {raw_reason}",
            )

    @commands.command(
        name="tempban",
        help="Temporarily bans a user.",
        description="Temporarily bans a user for the duration specified. Deletes the last 7 days worth of messages from the user.\n\n**Time formatting:**\n`s` or `second(s)`\n`m` or `minute(s)`\n`h` or `hour(s)`\n`d` or `day(s)`\n`w` or `week(s)`\n`M` or `month(s)`\n`Y` or `year(s)`\n\n**Example:** `tempban @User -d 5minutes -r 'Being naughty'` or `tempban @User 5d`\n**Note:** If your arguments contain spaces, you must wrap them in quotation marks.",
        usage="tempban <user> -d <duration> -r [reason] OR tempban <user> <duration>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(ban_members=True)
    @commands.bot_has_permissions(ban_members=True)
    ...
    @commands.command(
        help="Mass-bans a list of IDs specified.",
        description="Mass-bans a list of userIDs specified. Reason goes first, then a list of user IDs seperated by spaces.",
        usage="massban <reason> <userIDs>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(ban_members=True)
    @commands.bot_has_permissions(ban_members=True)
            ...
            for i, userid in enumerate(user_ids_conv):
                ...
                if i < 100:
                    try:
                        ...
                        await ctx.guild.ban(
                            member,
                            reason=f"Mass-banned by {ctx.author} ({ctx.author.id}): \n{reason}",
                        )
                    except:
                        failed += 1
                        if (
                            " - Error banning a user, userID is invalid or user is no longer member of the server."
                            not in errors
                        ):
                            errors.append(
                                " - Error banning a user, userID is invalid or user is no longer member of the server."
    ...
    @commands.command(
        help="Bans users based on criteria set. See command help for more.",
        description="""Bans a set of users based on the set of criteria specified. The command has advanced command-line syntax and is excellent for handling raids.
    ...
    **Example:**
    ...
    `smartban --reason "Bad person" --regex .*Username.* --joined 10`""",
        usage="smartban <args>",
    )
    @commands.guild_only()
    @commands.bot_has_permissions(ban_members=True)
    ...
    @commands.command(
        ...
        description="Bans a user then immediately unbans them, which means it will erase all messages from the user in the specified range.",
        usage="softban <user> [days-to-delete] [reason]",
    )
    @commands.check(has_mod_perms)
    ...
    @commands.group(
        ...
        description="Deletes up to 100 messages at once. You can optionally specify a user whose messages will be purged.",
    ...
    @purge.command(
        name="match",
        help="Delete messages containing the specified text.",
        usage="purge match <limit> <text>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    ...
    @purge.command(
        name="notext",
        help="Delete messages that do not contain text.",
        usage="purge notext <limit>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    ...
    @purge.command(
        name="startswith",
        help="Delete messages that start with the specified text.",
        usage="purge startswith <limit> <text>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    ...
    @purge.command(
        name="endswith",
        help="Delete messages that end to the specified text.",
        usage="purge endswith <limit> <text>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    ...
    @purge.command(
        name="links",
        aliases=["link"],
        help="Delete messages that contain links.",
        usage="purge links <limit>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    ...
    @purge.command(
        name="invites",
        aliases=["invite"],
        help="Delete messages that contain invites.",
        usage="purge invites <limit>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    ...
    @purge.command(
        name="images",
        aliases=["image"],
        help="Delete messages that contain attachments or images.",
        usage="purge images <limit>",
    )
    @commands.check(has_mod_perms)
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    ...
    @commands.command(
        aliases=["clr", "cleanup"],
        help="Cleans up the bot's messages.",
        description="Delete up to 50 of the bot's own responses in this channel. Defaults to 5.",
        usage="clear [limit]",
    )
    @commands.check(has_mod_perms)
    @commands.bot_has_permissions(manage_messages=True)
...
*Note: This user is not a member of this server*""",
    ...
    @commands.command(
        ...
        description="Provides information about a specified user. If they are in the server, more detailed information will be provided.\n\n__Note:__ To receive information about users outside this server, you must use their ID.",
        usage=f"whois <userID|userMention|userName>",
    )
    @commands.check(has_mod_perms)
    @commands.guild_only()
