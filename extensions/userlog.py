...
class Logging(commands.Cog):
    ...
    async def get_log_channel(self, event: str, guild_id: int) -> int:
        ...
        if event not in self.valid_log_events:
    ...
    async def log(
        self,
        event: str,
        logcontent: Union[str, discord.Embed],
        guild_id: int,
        ...
        try:
            ...
            async for entry in channel.guild.audit_logs():
                if entry.action == discord.AuditLogAction.channel_delete:
                    if (entry.target == channel or entry.target.id == channel.id) and (
        ...
        if before.communication_disabled_until != after.communication_disabled_until:
            ...
            if (
                ...
                await self.mod_cog.add_note(
                    after.id,
                    after.guild.id,
                    f"🔉 **Timeout removed by {moderator}:** {reason}",
                )
                embed = discord.Embed(
                ...
                await self.mod_cog.add_note(
                    after.id,
                    after.guild.id,
                    f"🔇 **Timed out by {moderator} until {discord.utils.format_dt(after.communication_disabled_until)}:** {reason}",
                )
                embed = discord.Embed(
