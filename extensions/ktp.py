...
class KeepOnTop(commands.Cog, name="Keep On Top"):
            ...
            if message.guild and message.guild.id in self.bot.whitelisted_guilds:
                ...
                if records:
                    for record in records:
                        if (
                            record["ktp_channel_id"] == message.channel.id
                            and record["ktp_content"] != message.content
                            ...
                            await self.bot.pool.execute(
                                """UPDATE ktp SET ktp_msg_id = $1 WHERE guild_id = $2 AND ktp_id = $3""",
                                new_top.id,
                                message.guild.id,
                                record["ktp_id"],
                            )
    ...
    @commands.group(
        aliases=["ktp"],
        help="Lists all keep-on-top messages. Subcommands can add/remove them.",
        description="Helps you list/manage keep-on-top messages. Keep-on-top messages are messages that are always the last message in the given channel, effectively being pinned.",
        usage="keepontop",
    ...
    @keepontop.command(
        name="add",
        aliases=["create", "new", "setup"],
        help="Initializes a setup to add a new keep-on-top message.",
        description="Initializes a setup to start configuring a new keep-on-top message. A server can have up to **3** keep-on-top messages. Takes no arguments.",
        usage="keepontop add",
    )
