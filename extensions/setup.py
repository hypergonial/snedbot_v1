...
class Setup(commands.Cog):
    ...
    async def moderation(self, ctx):
        ...
        async def setup_automod(self, ctx):
            embed=discord.Embed(title="🛠️ Auto-Moderation Setup", description='''Select your auto-moderation level!

            📘 - **Off**: Auto-Moderation features are disabled.
            📗 - **Light**: Spammers & Discord inviters without a role are warned/muted
            📒 - **Medium**: Spammers & Discord inviters are warned/muted
            📙 - **High:** Spammers & Discord inviters are softbanned
            📕 - **(╯°□°）╯︵ ┻━┻:** Spammers & Discord inviters are permabanned

            **__Note:__** The **`{prefix}raidmode`** command, when enabled, will automatically override this setting.'''.format(prefix=ctx.prefix), color=self.bot.embedBlue)
            msg = await ctx.channel.send(embed=embed)
            emojies = ['📘', '📗', '📒', '📙', '📕']
            for emoji in emojies:
                await msg.add_reaction(emoji)

            def reactioncheck(payload): #This has to be defined here again for some reason??
                return payload.message_id == msg.id and payload.user_id == ctx.author.id
            payload = await self.bot.wait_for('raw_reaction_add', timeout=60.0, check=reactioncheck)

            if str(payload.emoji) in emojies:
                automod_lvl = emojies.index(str(payload.emoji))
                async with self.bot.pool.acquire() as con:
                    await con.execute('''INSERT INTO mod_config (guild_id, automod_level) VALUES ($1, $2)
                    ON CONFLICT (guild_id) DO
                    UPDATE SET automod_level = $2''', ctx.guild.id, automod_lvl)
                #self.bot.cache['moderation'][ctx.guild.id]['automod_level'] = automod_lvl #Update the cache
                embed=discord.Embed(title="🛠️ Auto-Moderation Setup", description=f"✅ Auto-Moderation level updated!", color=self.bot.embedGreen)
                await ctx.channel.send(embed=embed)
            else:
                embed=discord.Embed(title=self.bot.errorEmojiTitle, description=self.bot.errorEmojiDesc, color=self.bot.errorColor)
                await ctx.send(embed=embed)
        
        async with self.bot.pool.acquire() as con:
            await con.execute('''INSERT INTO mod_config (guild_id) VALUES ($1) ON CONFLICT (guild_id) DO NOTHING''', ctx.guild.id)
            result = await con.fetch('''SELECT * FROM mod_config WHERE guild_id = $1''', ctx.guild.id)
        
        automod_levels = ["Off", "Light", "Medium", "High", "(╯°□°）╯︵ ┻━┻"]

        if ctx.guild.get_role(result[0].get('mute_role_id')): mute_mention = ctx.guild.get_role(result[0].get('mute_role_id')).mention
        else: mute_mention = "Not set"

        if result[0].get('is_raidmode'): automod_level = 4
        else: automod_level = result[0].get('automod_level')

        embed=discord.Embed(title="🛠️ Moderation Config", color=self.bot.embedBlue, description=f"""Which aspect do you wish to configure?
        **🔇 - Mute role** (*{mute_mention}*)
        **🤖 - Auto-Moderation** (*{automod_levels[automod_level]}*)""")
        msg = await ctx.channel.send(embed=embed)
        ...
        if str(payload.emoji) == "🔇":
            ...
            if not result[0].get('is_raidmode'):
                await setup_automod(self, ctx)
            else:
                embed=discord.Embed(title="❌ Error: Raidmode is on", description="Please disable raidmode before attempting to change auto-moderation settings.", color=self.bot.errorColor)
                await ctx.send(embed=embed)
